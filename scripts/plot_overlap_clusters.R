...
get_mt_reduction <- function(x, seur1, seur2, thresh=0.25, ct_col1="seurat_clusters", ct_col2="seurat_clusters"){
	...
	mt1 <- sapply(ct1, function(x){mean(seur1@meta.data[seur1$seurat_clusters == x,"percent.mt"])})
	mt2 <- sapply(ct2, function(x){mean(seur2@meta.data[seur2$seurat_clusters == x,"percent.mt"])})
...
mt_ED_ad <- sapply(rownames(ad_overlap), function(x){mean(seur_ED_ad@meta.data[seur_ED_ad@meta.data[,ctype] == x,"percent.mt"])})
mt_diem_ad <- sapply(colnames(ad_overlap), function(x){mean(seur_diem_ad@meta.data[seur_diem_ad@meta.data[,ctype] == x,"percent.mt"])})

mb_overlap <- comp_clust(seur_ED_mb, seur_diem_mb, ct_col1=ctype, ct_col2=ctype)
mt_ED_mb <- sapply(rownames(mb_overlap), function(x){mean(seur_ED_mb@meta.data[seur_ED_mb@meta.data[,ctype] == x,"percent.mt"])})
mt_diem_mb <- sapply(colnames(mb_overlap), function(x){mean(seur_diem_mb@meta.data[seur_diem_mb@meta.data[,ctype] == x,"percent.mt"])})

at_overlap <- comp_clust(seur_ED_at, seur_diem_at, ct_col1=ctype, ct_col2=ctype)
mt_ED_at <- sapply(rownames(at_overlap), function(x){mean(seur_ED_at@meta.data[seur_ED_at@meta.data[,ctype] == x,"percent.mt"])})
mt_diem_at <- sapply(colnames(at_overlap), function(x){mean(seur_diem_at@meta.data[seur_diem_at@meta.data[,ctype] == x,"percent.mt"])})

pe1 <- overlap_graph(ad_overlap, mt_ED_ad, mt_diem_ad, labels1="EmptyDrops", labels2="DIEM", main="DiffPA", col_title="MT%")
pe2 <- overlap_graph(mb_overlap, mt_ED_mb, mt_diem_mb, labels1="EmptyDrops", labels2="DIEM", main="Mouse Brain", col_title="MT%")
pe3 <- overlap_graph(at_overlap, mt_ED_at, mt_diem_at, labels1="EmptyDrops", labels2="DIEM", main="Adipose Tissue", col_title="MT%")
...
r_ed_allp <- bp(r_ed_all, y=expression("MT%"[DIEM] - "MT%"[EmptyDrops]))

# Cluster overlap between quantile and DIEM
ctype="RNA_snn_res.0.8"
ad_overlap <- comp_clust(seur_quant_ad, seur_diem_ad, ct_col1=ctype, ct_col2=ctype)
mt_quant_ad <- sapply(rownames(ad_overlap), function(x){mean(seur_quant_ad@meta.data[seur_quant_ad@meta.data[,ctype] == x,"percent.mt"])})
mt_diem_ad <- sapply(colnames(ad_overlap), function(x){mean(seur_diem_ad@meta.data[seur_diem_ad@meta.data[,ctype] == x,"percent.mt"])})

mb_overlap <- comp_clust(seur_quant_mb, seur_diem_mb, ct_col1=ctype, ct_col2=ctype)
mt_quant_mb <- sapply(rownames(mb_overlap), function(x){mean(seur_quant_mb@meta.data[seur_quant_mb@meta.data[,ctype] == x,"percent.mt"])})
mt_diem_mb <- sapply(colnames(mb_overlap), function(x){mean(seur_diem_mb@meta.data[seur_diem_mb@meta.data[,ctype] == x,"percent.mt"])})

at_overlap <- comp_clust(seur_quant_at, seur_diem_at, ct_col1=ctype, ct_col2=ctype)
mt_quant_at <- sapply(rownames(at_overlap), function(x){mean(seur_quant_at@meta.data[seur_quant_at@meta.data[,ctype] == x,"percent.mt"])})
mt_diem_at <- sapply(colnames(at_overlap), function(x){mean(seur_diem_at@meta.data[seur_diem_at@meta.data[,ctype] == x,"percent.mt"])})

pq1 <- overlap_graph(ad_overlap, mt_quant_ad, mt_diem_ad, labels1="Quantile", labels2="DIEM", main="DiffPA", col_title="MT%")
pq2 <- overlap_graph(mb_overlap, mt_quant_mb, mt_diem_mb, labels1="Quantile", labels2="DIEM", main="Mouse Brain", col_title="MT%")
pq3 <- overlap_graph(at_overlap, mt_quant_at, mt_diem_at, labels1="Quantile", labels2="DIEM", main="Adipose Tissue", col_title="MT%")
...
r_q_allp <- bp(r_q_all, y=expression("MT%"[DIEM] - "MT%"[Quantile]))
...
ggarrange(ggarrange(p_blank, pq1, pq2, pq3, p_blank, pe1, pe2, pe3, 
                    ncol=8, labels=c("a", "", "", "", "b", "", "", ""), 
                    font.label = list(size = 28, face="bold"),
                    widths=c(.25,1,1,1,.25,1,1,1)),
          ggarrange(r_q_allp, r_ed_allp, nrow=1, 
                    labels=c("c", "d"),
                    font.label = list(size = 28, face="bold")), ncol=1, heights=c(.6,.4))
dev.off()
...
mtq_quant_mb <- sapply(rownames(mbq_overlap), function(x){mean(seur_quant_mb@meta.data[seur_quant_mb@meta.data[,ctype] == x,"percent.mt"])})
mtq_diem_mb <- sapply(colnames(mbq_overlap), function(x){mean(seur_diem_mb@meta.data[seur_diem_mb@meta.data[,ctype2] == x,"percent.mt"])})

mb_overlap <- comp_clust(seur_ED_mb, seur_diem_mb, ct_col1=ctype, ct_col2=ctype2)
mt_quant_mb <- sapply(rownames(mb_overlap), function(x){mean(seur_ED_mb@meta.data[seur_ED_mb@meta.data[,ctype] == x,"percent.mt"])})
mt_diem_mb <- sapply(colnames(mb_overlap), function(x){mean(seur_diem_mb@meta.data[seur_diem_mb@meta.data[,ctype2] == x,"percent.mt"])})

atq_overlap <- comp_clust(seur_quant_at, seur_diem_at, ct_col1=ctype, ct_col2=ctype2)
atq_quant_at <- sapply(rownames(atq_overlap), function(x){mean(seur_quant_at@meta.data[seur_quant_at@meta.data[,ctype] == x,"percent.mt"])})
atq_diem_at <- sapply(colnames(atq_overlap), function(x){mean(seur_diem_at@meta.data[seur_diem_at@meta.data[,ctype2] == x,"percent.mt"])})

at_overlap <- comp_clust(seur_ED_at, seur_diem_at, ct_col1=ctype, ct_col2=ctype2)
mt_ED_at <- sapply(rownames(at_overlap), function(x){mean(seur_ED_at@meta.data[seur_ED_at@meta.data[,ctype] == x,"percent.mt"])})
mt_diem_at <- sapply(colnames(at_overlap), function(x){mean(seur_diem_at@meta.data[seur_diem_at@meta.data[,ctype2] == x,"percent.mt"])})


ps1 <- overlap_graph(mbq_overlap, mtq_quant_mb, mtq_diem_mb, labels1="Quantile", labels2="DIEM (Res. 2)", main="Mouse Brain", col_title="MT%")
ps2 <- overlap_graph(mb_overlap, mt_ED_mb, mt_diem_mb, labels1="EmptyDrops", labels2="DIEM (Res. 2)", main="Mouse Brain", col_title="MT%")
ps3 <- overlap_graph(atq_overlap, atq_quant_at, atq_diem_at, labels1="Quantile", labels2="DIEM (Res. 2)", main="Adipose Tissue", col_title="MT%")
ps4 <- overlap_graph(at_overlap, mt_ED_at, mt_diem_at, labels1="EmptyDrops", labels2="DIEM (Res. 2)", main="Adipose Tissue", col_title="MT%")
